      ...
      {error || !scopeData ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-2 p-6 rounded-2xl border bg-card text-card-foreground shadow-premium flex flex-col justify-between gap-4">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  {scopeData.drift_detected 
                    ? "Warning: Unauthorized features or modifications detected. Please review recently closed tasks."
              ...
              <div className="grid grid-cols-2 gap-4 pt-4 border-t text-xs">
                <div>
                  <span className="text-muted-foreground block">Compliance Rating</span>
                  <span className="font-bold text-lg text-primary">{scopeData.scope_alignment_score}%</span>
                </div>
                <div>
                  <span className="text-muted-foreground block">Requirements Audit</span>
                  <span className="font-bold text-lg text-foreground">
                    {scopeData.implemented_requirements} / {scopeData.total_requirements} Met
                  </span>
                </div>
              </div>
            </div>
            ...
            <ProgressCard
              title="Scope Alignment Score"
              percentage={scopeData.scope_alignment_score}
              label={`${scopeData.implemented_requirements} / ${scopeData.total_requirements} elements complete`}
              sublabel="target: 100%"
            />
          </div>
